...
import create_html
from html_models.check_login import *
from html_models.change_user_pd import *
from html_models.check_register import add_user
...
def login_base():
    ...
    if 'username' in msg and 'password' in msg:
        ...
        user_in_yml = dict(get_user_pwd())
        if username in user_in_yml.keys():
            if password == user_in_yml[username]:
                session['user'] = msg['username']
                session['password'] = msg['password']
                return create_html.jump2Html('/', '登录成功', 1)
            else:
                login_massage = "登录失败！(温馨提示：密码错误，请输入正确密码)"
                return create_html.jump2Html('/login', login_massage, 3)
        elif is_null(username, password):
            login_massage = "登录失败！(温馨提示：账号和密码是必填)"
            return create_html.jump2Html('/login', login_massage, 3)
            ...
            login_massage = "登录失败！(温馨提示：不存在该用户，请先注册)"
            return create_html.jump2Html('/login', login_massage, 3)
...
def register_base():
    if request.method == 'POST':
        ...
        if is_null(username, password):
            login_massage = "温馨提示：账号和密码是必填项"
            return create_html.jump2Html('/register', login_massage, 3)
        elif exist_user(username) or username in user_in_yml.keys():
            login_massage = "温馨提示：用户已存在，请直接登录"
            # return redirect(url_for('user_login'))
            return create_html.jump2Html('/register', login_massage, 3)
        else:
            add_user(request.form['username'], request.form['password'])
            login_massage = "注册成功！3s后为您跳转至登录页面..."
            return create_html.jump2Html('/login', login_massage, 3)
    return render_template('register.html', web_name=create_html.get_web_name())
...
@app.before_request
def check_login():
    Start_enters = ['/file','/login','/static','/register']
    for i in Start_enters:
        if request.path.startswith(i): return
    username = session.get('user')
    password = session.get('password')
    if username not in get_user_pwd() or not is_existed(username, password):
...
def change_user_pwd():
    ...
    if 'new_username' in msg and 'new_password' in msg:
        ...
        if exist_user(new_username) or new_username in user_in_yml.keys():
            massage = "温馨提示：该用户名已存在，请更改用户名"
            return create_html.jump2Html('/change_user_pwd', massage,3)
        elif is_null(new_username,new_password):
            message = "温馨提示：用户名或密码不能为空"
            return create_html.jump2Html('/change_user_pwd',message,3)
        elif len(new_username) > 12 or len(new_password) > 12:
            message = "温馨提示：用户名或密码长度不能超过12位"
            return create_html.jump2Html('/change_user_pwd',message,3)
        else:
            # 更改yaml文件中的用户信息
            with open("./run/user.yaml") as f:
                user_data = yaml.load(f,yaml.Loader)
            user_data[new_username] = new_password
            user_data.pop(session.get('user'))
            with open('./run/user.yaml', 'w') as f:
                yaml.dump(user_data, f)
            # 更改数据库中的用户信息
            change_user(session.get('user'),new_username,new_password)
            return create_html.jump2Html('/settings_list', '更改用户名和密码成功', 2)
...
@app.route('/edit_detectSetting', methods=["GET", "POST"])
def edit_detectSetting():
    msg = get_web_get()
    # print(msg)
    assert 'name' in msg
    if os.path.isfile('./settings/%s/data_dir.txt' % msg['name']):
        data_dir = open('./settings/%s/data_dir.txt' % msg['name']).read()
        return create_html.getDetectSettingHtml(data_dir, msg['name'])
    else:
        return create_html.jump2Html('/model_pred', 'error', 1)
...
def model_pred():
    ...
    if request.method == "POST":
        ...
        upload_path = os.path.join(basepath, 'static', secure_filename(img_file.filename))  # 注意：没有的文件夹一定要先创建，不然会提示没有该路径
        img_file.save(upload_path)

        image_data = cv2.imdecode(np.fromfile(upload_path,np.uint8),-1)
        response = make_response(image_data)
        response.headers['Content-Type'] = 'image/png'
        return response
    return create_html.Model_predHtml(msg['name'])

@app.route('/save_detectSettings', methods=["GET", "POST"])
def save_detectSettiings():
    msg = get_webform()
    for name in msg:
        print(name, msg[name])
    output_dir = os.path.abspath("./settings/%s/" % msg['name']).replace('\\', '/')
    ...
    if not output_dir.endswith('/'):
        output_dir += '/'
    msg_to_save = {
        "nms_thres": float(msg['nms_thres']),
        "source": msg['data_dir'],
        "confidence_thres": float(msg['conf_thres']),
        "input_size": int(msg['input_size']),
        "fp16": 'fp16' in msg,
        "device": 'gpu',
        "gpu_num": int(msg['gpu_num']),
        "weight_size": msg['model_size'],
        "show_image": msg["show_image"] == "True" if "show_image" in msg else True,
        "output_dir": output_dir,
        "model_path": pretrained_file if use_pretrained_file else "no file selected",
        "auto_choose_device": 'auto_choose_device' in msg,
        "is_trt_file": msg["is_trt_file"] if "is_trt_file" == "True" in msg else False,
        "source_type": msg["source_type"] if "source_type" in msg else 'image',
        "pred_name": "output",
        "use_pretrained_weight": use_pretrained_file,
        "start_with_pause": msg["start_with_pause"] == "True" if "start_with_pause" in msg else False,
        "save_image": msg["save_image"] == "True" if "save_image" in msg else True,
        "classes_file": '%sclasses.txt' % output_dir,
    }
    class_string = msg["classes"].replace('\n','')
    if class_string.endswith("\n"):
        class_string = class_string[:-1]
    print(class_string)
    if len(class_string):
        open('%sclasses.txt' % output_dir, "w").write(class_string)
    try:
    ...
    # return ret
    yaml.dump(msg_to_save, open('%sdetectSet.yaml' % output_dir, 'w'), yaml.Dumper)
    return create_html.jump2Html('/model_pred?name=%s'%msg['name'], ['success'], 1)
...
def train_latest_msg():
    msg = get_web_get()
    if 'name' in msg:
        if create_html.extract_latest_train_msg(msg['name']) is not None:
            return create_html.extract_latest_train_msg(msg['name'])
        else:
            return "{}暂未生成".format(msg['name'])
    return create_html.jump2Html('/settings_list', 'Wrong Request!', 1)
...
if __name__ == '__main__':
    ...
    app.run(host='100.87.188.188', port=args.port, debug=args.debug)
