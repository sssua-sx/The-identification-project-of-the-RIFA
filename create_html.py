...
def extract_latest_train_msg(setting_dir, epoch: int or None = None):
    ...
    now_epoch_dir = os.path.join(setting_dir, 'output', 'epochs', str(now_epoch).zfill(4))
    if os.path.exists(os.path.join(now_epoch_dir, 'latest')):
...
def getSettingsList():
    ...
    for i, item in enumerate(list_show):
        ...
        <td><center>%s</center></td>
            ...
            '<a href="/model_pred?name=%s">模型预测</a>&nbsp;&nbsp;'
            '%s'
            '%s' % (
                item['name'],
                item['name'],
...
def getDetectSettingHtml(data_root, settings_dir):
    import os
    import yaml
    import sys
    ...
    if os.path.isfile('./settings/%s/detectSet.yaml' % settings_dir):
        settings_exist = True
        msg = yaml.load(open('./settings/%s/detectSet.yaml' % settings_dir), yaml.Loader)
    print(msg)
    #train_dir_index = dir_list.index(msg['train_dataset_path'].split('/')[-1]) if settings_exist else 0
    source_type = msg['source_type'] if settings_exist else 'image_dir'
    #val_dir_index = dir_list.index(msg['val_dataset_path'].split('/')[-1]) if settings_exist else 0
    save_image = msg['save_image'] if settings_exist else False
    #train_json_index = anno_list.index(msg['train_annotation_file'].split('/')[-1]) if settings_exist else 0
    is_trt_file = msg['is_trt_file'] if settings_exist else False
    #val_json_index = anno_list.index(msg['val_annotation_file'].split('/')[-1]) if settings_exist else 0
    start_with_pause = msg['start_with_pause'] if settings_exist else False
    model_size = msg['weight_size'] if settings_exist else 's'
    use_gpu_num = msg['gpu_num'] if settings_exist else 0
    conf_thres = msg['confidence_thres'] if settings_exist else 0.15
    nms_thres = msg['nms_thres'] if settings_exist else 0.3
    input_size = msg['input_size'] if settings_exist else 640
    pretrained_file = '' if not settings_exist else '' if msg['model_path'] == 'no file selected' else msg[
        'model_path']
    fp16 = msg['fp16'] if settings_exist else False
    auto_choose_device = msg['auto_choose_device'] if settings_exist else True
    #backbone = msg["backbone_type"] if "backbone_type" in msg else "origin"
    show_image = msg['show_image'] if settings_exist else False
    ...
    save_image_string = ""
    is_trt_file_string = ""
    start_with_pause_string = ""
    model_size_string = ""
    gpu_string = ""
    class_string = ""
    show_image_string = ""

    if os.path.exists('./settings/%s/classes.txt' % settings_dir):
        class_string = open('./settings/%s/classes.txt' % settings_dir).read()
        if class_string.endswith("\n"):
            class_string = class_string[:-1].replace('\n', '')
    source_type_list = ['image_dir','cam','image']
    TF_list = [True,False]
    for type in source_type_list:
        source_type_string += "<option value='%s'%s>%s</option>" % (
            type,
            ' selected'
            if type == source_type
            else '',
    ...
    for TF in TF_list:
        save_image_string += "<option value=%s%s>%s</option>" % (
            TF,
            ' selected'
            if TF == save_image
            else '',
        ...
        is_trt_file_string += "<option value=%s%s>%s</option>" % (
            TF,
            ' selected'
            if TF == is_trt_file
            else '',
        ...
        start_with_pause_string += "<option value=%s%s>%s</option>" % (
            TF,
            ' selected'
            if TF == start_with_pause
            else '',
            TF
        )
        show_image_string += "<option value=%s%s>%s</option>" % (
            TF,
            ' selected'
            if TF == show_image
            else '',
    ...
    data = {
        ...
        "data_root": data_root,
        "source_type_string": source_type_string,
        "save_image_string": save_image_string,
        "is_trt_file_string": is_trt_file_string,
        "start_with_pause_string": start_with_pause_string,
        "show_image_string": show_image_string,
        ...
        "check1": ' checked' if auto_choose_device else '',
        "class_string": class_string
    }
    return render_template("detect_setting.html", web_name=get_web_name(), **data)
...
def Model_predHtml(name: str):
    return render_template("model_pred.html", web_name=get_web_name(), name=name)
